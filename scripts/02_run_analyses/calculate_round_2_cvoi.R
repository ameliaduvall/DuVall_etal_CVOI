...
## create round 2 df 
source(here("scripts", "format_data","load_round_2.R"))
...
# saveRDS(assp, here("data", "assp_round2.rds"))
...
for(i in 1:length(hypothesis)){
  ...
  cvoi.lci[i]<-as.numeric(quantile((assp$cvoi[which(assp$hypothesis==hypothesis[i])]), 0.05))
  cvoi.uci[i]<-as.numeric(quantile((assp$cvoi[which(assp$hypothesis==hypothesis[i])]), 0.95))
  red.lci[i]<-as.numeric(quantile((assp$red[which(assp$hypothesis==hypothesis[i])]), 0.05, na.rm = T))
  red.uci[i]<-as.numeric(quantile((assp$red[which(assp$hypothesis==hypothesis[i])]), 0.95, na.rm = T))
}

assp_r2_summary <- data.frame(Hypothesis=unique(n_ex_rel$hypothesis),
                              Name= unique(n_ex_rel$name), 
                              CVOI.mean= as.numeric(assp_r2_means$cvoi),
                              CVOI.lci=cvoi.lci,
                              CVOI.uci=cvoi.uci,
                              Red.mean= as.numeric(assp_r2_means$red),
                              Red.lci=red.lci,
                              Red.uci=red.uci,
                              RelA=as.numeric(assp_r2_means$rel_a),
                              RelB=as.numeric(assp_r2_means$rel_b),
                              Mag=as.numeric(assp_r2_means$mag),
                              Rel_Both=as.numeric(assp_r2_means$rel_both),
                              Num_experts = n_ex_rel$n_ex)

#write.csv(assp_r2_summary, here("results", "assp_r2_summary.csv"))
...
brpe <- full_df %>% 
  ...
  mutate(rel_both = ((rel_a*rel_b)-0/(4-0)),
         ...
         red = ifelse((rel_a == 0 & rel_b == 0 & mag == 0), NA, red)) %>%
  select(expert, category, hypothesis, code, name, species, rel_a, rel_b, rel_both, mag, red, cvoi, conf)
# saveRDS(brpe, here("data", "brpe_round2.rds"))
...
for(i in 1:length(hypothesis)){
  ...
  cvoi.lci[i]<-as.numeric(quantile((brpe$cvoi[which(brpe$hypothesis==hypothesis[i])]), 0.05))
  cvoi.uci[i]<-as.numeric(quantile((brpe$cvoi[which(brpe$hypothesis==hypothesis[i])]), 0.95))
  red.lci[i]<-as.numeric(quantile((brpe$red[which(brpe$hypothesis==hypothesis[i])]), 0.05, na.rm = T))
  red.uci[i]<-as.numeric(quantile((brpe$red[which(brpe$hypothesis==hypothesis[i])]), 0.95, na.rm = T))
}

brpe_r2_summary <- data.frame(Hypothesis=unique(n_ex_rel$hypothesis),
                              Name= unique(n_ex_rel$name), 
                              CVOI.mean= as.numeric(brpe_r2_means$cvoi),
                              CVOI.lci=cvoi.lci,
                              CVOI.uci=cvoi.uci,
                              Red.mean= as.numeric(brpe_r2_means$red),
                              Red.lci=red.lci,
                              Red.uci=red.uci,
                              RelA=as.numeric(brpe_r2_means$rel_a),
                              RelB=as.numeric(brpe_r2_means$rel_b),
                              Mag=as.numeric(brpe_r2_means$mag),
                              Rel_Both=as.numeric(brpe_r2_means$rel_both),
                              Num_experts = n_ex_rel$n_ex)

#write.csv(brpe_r2_summary, here("results", "brpe_r2_summary.csv"))
...
caau <- full_df %>% 
  ...
  mutate(rel_both = ((rel_a*rel_b)-0/(4-0)),
         ...
         red = ifelse((rel_a == 0 & rel_b == 0 & mag == 0), NA, red)) %>%
  select(expert, category, hypothesis, code, name, species, rel_a, rel_b, rel_both, mag, red, cvoi, conf)
# saveRDS(caau, here("data", "caau_round2.rds"))
...
for(i in 1:length(hypothesis)){
  ...
  cvoi.lci[i]<-as.numeric(quantile((caau$cvoi[which(caau$hypothesis==hypothesis[i])]), 0.05))
  cvoi.uci[i]<-as.numeric(quantile((caau$cvoi[which(caau$hypothesis==hypothesis[i])]), 0.95))
  red.lci[i]<-as.numeric(quantile((caau$red[which(caau$hypothesis==hypothesis[i])]), 0.05, na.rm = T))
  red.uci[i]<-as.numeric(quantile((caau$red[which(caau$hypothesis==hypothesis[i])]), 0.95, na.rm = T))
}

caau_r2_summary <- data.frame(Hypothesis=unique(n_ex_rel$hypothesis),
                              Name= unique(n_ex_rel$name), 
                              CVOI.mean= as.numeric(caau_r2_means$cvoi),
                              CVOI.lci=cvoi.lci,
                              CVOI.uci=cvoi.uci,
                              Red.mean= as.numeric(caau_r2_means$red),
                              Red.lci=red.lci,
                              Red.uci=red.uci,
                              RelA=as.numeric(caau_r2_means$rel_a),
                              RelB=as.numeric(caau_r2_means$rel_b),
                              Mag=as.numeric(caau_r2_means$mag),
                              Rel_Both=as.numeric(caau_r2_means$rel_both),
                              Num_experts = n_ex_rel$n_ex)

#write.csv(caau_r2_summary, here("results", "caau_r2_summary.csv"))
...
scmu <- full_df %>% 
  ...
  mutate(rel_both = ((rel_a*rel_b)-0/(4-0)),
         ...
         red = ifelse((rel_a == 0 & rel_b == 0 & mag == 0), NA, red)) %>%
  select(expert, category, hypothesis, code, name, species, rel_a, rel_b, rel_both, mag, red, cvoi, conf)
# saveRDS(scmu, here("data", "scmu_round2.rds"))
...
for(i in 1:length(hypothesis)){
  ...
  cvoi.lci[i]<-as.numeric(quantile((scmu$cvoi[which(scmu$hypothesis==hypothesis[i])]), 0.05))
  cvoi.uci[i]<-as.numeric(quantile((scmu$cvoi[which(scmu$hypothesis==hypothesis[i])]), 0.95))
  red.lci[i]<-as.numeric(quantile((scmu$red[which(scmu$hypothesis==hypothesis[i])]), 0.05, na.rm = T))
  red.uci[i]<-as.numeric(quantile((scmu$red[which(scmu$hypothesis==hypothesis[i])]), 0.95, na.rm = T))
}

scmu_r2_summary <- data.frame(Hypothesis=unique(n_ex_rel$hypothesis),
                              Name= unique(n_ex_rel$name), 
                              CVOI.mean= as.numeric(scmu_r2_means$cvoi),
                              CVOI.lci=cvoi.lci,
                              CVOI.uci=cvoi.uci,
                              Red.mean= as.numeric(scmu_r2_means$red),
                              Red.lci=red.lci,
                              Red.uci=red.uci,
                              RelA=as.numeric(scmu_r2_means$rel_a),
                              RelB=as.numeric(scmu_r2_means$rel_b),
                              Mag=as.numeric(scmu_r2_means$mag),
                              Rel_Both=as.numeric(scmu_r2_means$rel_both),
                              Num_experts = n_ex_rel$n_ex)

#write.csv(scmu_r2_summary, here("results", "scmu_r2_summary.csv"))
...
snpl <- full_df %>% 
  ...
  mutate(rel_both = ((rel_a*rel_b)-0/(4-0)),
         ...
         red = ifelse((rel_a == 0 & rel_b == 0 & mag == 0), NA, red)) %>%
  select(expert, category, hypothesis, code, name, species, rel_a, rel_b, rel_both, mag, red, cvoi, conf)
# saveRDS(snpl, here("data", "snpl_round2.rds"))
...
for(i in 1:length(hypothesis)){
  ...
  cvoi.lci[i]<-as.numeric(quantile((snpl$cvoi[which(snpl$hypothesis==hypothesis[i])]), 0.05))
  cvoi.uci[i]<-as.numeric(quantile((snpl$cvoi[which(snpl$hypothesis==hypothesis[i])]), 0.95))
  red.lci[i]<-as.numeric(quantile((snpl$red[which(snpl$hypothesis==hypothesis[i])]), 0.05, na.rm = T))
  red.uci[i]<-as.numeric(quantile((snpl$red[which(snpl$hypothesis==hypothesis[i])]), 0.95, na.rm = T))
}

snpl_r2_summary <- data.frame(Hypothesis=unique(n_ex_rel$hypothesis),
                              Name= unique(n_ex_rel$name), 
                              CVOI.mean= as.numeric(snpl_r2_means$cvoi),
                              CVOI.lci=cvoi.lci,
                              CVOI.uci=cvoi.uci,
                              Red.mean= as.numeric(snpl_r2_means$red),
                              Red.lci=red.lci,
                              Red.uci=red.uci,
                              RelA=as.numeric(snpl_r2_means$rel_a),
                              RelB=as.numeric(snpl_r2_means$rel_b),
                              Mag=as.numeric(snpl_r2_means$mag),
                              Rel_Both=as.numeric(snpl_r2_means$rel_both),
                              Num_experts = n_ex_rel$n_ex)

#write.csv(snpl_r2_summary, here("results", "snpl_r2_summary.csv"))
...
wegu <- full_df %>% 
  ...
  mutate(rel_both = ((rel_a*rel_b)-0/(4-0)),
         ...
         red = ifelse((rel_a == 0 & rel_b == 0 & mag == 0), NA, red)) %>%
  select(expert, category, hypothesis, code, name, species, rel_a, rel_b, rel_both, mag, red, cvoi, conf)
# saveRDS(wegu, here("data", "wegu_round2.rds"))
...
for(i in 1:length(hypothesis)){
  ...
  cvoi.lci[i]<-as.numeric(quantile((wegu$cvoi[which(wegu$hypothesis==hypothesis[i])]), 0.05))
  cvoi.uci[i]<-as.numeric(quantile((wegu$cvoi[which(wegu$hypothesis==hypothesis[i])]), 0.95))
  red.lci[i]<-as.numeric(quantile((wegu$red[which(wegu$hypothesis==hypothesis[i])]), 0.05, na.rm = T))
  red.uci[i]<-as.numeric(quantile((wegu$red[which(wegu$hypothesis==hypothesis[i])]), 0.95, na.rm = T))
}

wegu_r2_summary <- data.frame(Hypothesis=unique(n_ex_rel$hypothesis),
                              Name= unique(n_ex_rel$name), 
                              CVOI.mean= as.numeric(wegu_r2_means$cvoi),
                              CVOI.lci=cvoi.lci,
                              CVOI.uci=cvoi.uci,
                              Red.mean= as.numeric(wegu_r2_means$red),
                              Red.lci=red.lci,
                              Red.uci=red.uci,
                              RelA=as.numeric(wegu_r2_means$rel_a),
                              RelB=as.numeric(wegu_r2_means$rel_b),
                              Mag=as.numeric(wegu_r2_means$mag),
                              Rel_Both=as.numeric(wegu_r2_means$rel_both),
                              Num_experts = n_ex_rel$n_ex)

#write.csv(wegu_r2_summary, here("results", "wegu_r2_summary.csv"))

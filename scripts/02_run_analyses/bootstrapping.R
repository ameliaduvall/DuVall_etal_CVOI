## Bootstrapping
## Includes: By Species, Global Hypotheses, and Global Threat Categories
...
## bootstrap function
run_cvoi_boot_analysis <- function(df, group_vars, mode = "standard", n_reps = 1000) {
  ...
  boot_reps <- map_df(1:n_reps, function(i) {
    ...
    resampled_data <- df %>%
      ...
      group_modify(~ {
        ...
        ## pull raw scores and allow duplicates 
    ...
    iteration_means <- resampled_data %>%
      ...
      summarize(
        n_exp = n(), 
        n_rel  = sum(rel_a > 0, na.rm = TRUE),
        spp_hypo_cvoi = sum(cvoi, na.rm = TRUE) / n_exp,
        spp_hypo_red  = sum(red, na.rm = TRUE) / n_rel,
        .groups = "drop"
      )
    ...
    if (mode == "by_species") {
      ...
      ## mean of means across all 6 species for each hypothesis
      iteration_means %>%
      ...
      ## mean of means across all 6 species for each IUCN Category
      iteration_means %>%
        group_by(iucn_name) %>%
        summarize(rep_cvoi = mean(spp_hypo_cvoi, na.rm = TRUE),
                  rep_red  = mean(spp_hypo_red, na.rm = TRUE),
  ...
  ## get quadrant boundaries based on medians of the means
...
boot_by_species <- run_cvoi_boot_analysis(data_prepped, 
                                          group_vars = c("species", "hypothesis", "code", "name", "iucn_name"), 
                                          mode = "by_species")

## bootstrap across species by hypothesis
boot_global_hypo <- run_cvoi_boot_analysis(data_prepped, 
                                           group_vars = c("hypothesis", "code", "name", "iucn_name"), 
                                           mode = "global_hypo")

## bootstrap across species by IUCN threat category
boot_global_iucn <- run_cvoi_boot_analysis(data_prepped, 
                                           group_vars = "iucn_name", 
                                           mode = "global_iucn")

## save results
write.csv(boot_by_species,  here("results", "boot_results_by_species.csv"), row.names = FALSE)
write.csv(boot_global_hypo, here("results", "boot_results_global_hypo.csv"), row.names = FALSE)
write.csv(boot_global_iucn, here("results", "boot_results_global_iucn.csv"), row.names = FALSE)
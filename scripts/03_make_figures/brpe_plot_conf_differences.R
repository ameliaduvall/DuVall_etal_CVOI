...
## brpe ####
brpe_boot1 <- readRDS(here("results", "brpe_bootstrapped.rds")) %>%
  as.data.frame() %>%
  mutate(weight = c('full'))
brpe_boot2 <- readRDS(here("results", "brpe_bootstrapped_linear_conf.rds")) %>%
  as.data.frame() %>%
  mutate(weight = c('linear'))

brpe_df <- bind_rows(brpe_boot1, brpe_boot2) %>%
  left_join(., full_df, by = c("hypothesis", "name"))
brpe_conf <- bind_rows(brpe_boot1, brpe_boot2) %>%
  pivot_wider(names_from = "weight", values_from = c(mean_cvoi, var_cvoi, mean_red, var_red)) %>%
  left_join(., full_df, by = c("hypothesis", "name"))

ggplot() +
  geom_segment(data = brpe_conf, aes(x = mean_cvoi_full, y = mean_red_full,
                                    xend = mean_cvoi_linear, yend = mean_red_linear, color = category),
              arrow = arrow(length = unit(0.02, "npc")),
              size = 1.5) +
  geom_point(data = brpe_df %>% filter(weight == "full"),
             aes(x = mean_cvoi, y = mean_red, color = category),
             size = 4, stroke = 0, shape = 16) +
  geom_point(data = brpe_df %>% filter(weight == "linear"),
             aes(x = mean_cvoi, y = mean_red, color = category),
             shape = 4, size = 4) +
  geom_vline(aes(xintercept = median(brpe_df$mean_cvoi)), 
             linetype = 'dashed',color = 'black',linewidth = 0.75) +
  geom_hline(aes(yintercept = median(brpe_df$mean_red, na.rm = T)), 
             linetype = 'dashed', color = 'black',linewidth = 0.75) +
  geom_label_repel(data = brpe_df %>% filter(weight == "full"),
...
#ggsave(here("figures", "final", "brpe_conf_cvoi.png"), width = 12, height = 8)  

## compare priority rankings ####
## look at priority rankings for full confidence
median_cvoi <- median(brpe_boot1$mean_cvoi)
median_red <- median(brpe_boot1$mean_red, na.rm = TRUE)

brpe_rankings <- brpe_boot1%>%
...
median_cvoi2 <- median(brpe_boot2$mean_cvoi)
median_red2 <- median(brpe_boot2$mean_red, na.rm = TRUE)

brpe_rankings2 <- brpe_boot2 %>%
  mutate(quadrat = case_when(
...
## compare df
chk <- full_join(brpe_rankings, brpe_rankings2, by = c("hypothesis", "name"))

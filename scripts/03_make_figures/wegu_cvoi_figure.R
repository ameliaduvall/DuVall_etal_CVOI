...
ggplot(wegu_df, aes(x = mean_cvoi, y = mean_red)) + 
  ...
  geom_linerange(aes(ymin = red_y_min, ymax = red_y_max, color = category), 
                 alpha = 1, linetype = "solid") +
  geom_vline(aes(xintercept = median(wegu_df$mean_cvoi)), linetype = 'dashed',color = 'black',linewidth = 0.75) +
  geom_hline(aes(yintercept = median(wegu_df$mean_red, na.rm = T)), linetype = 'dashed', color = 'black',linewidth = 0.75) +
...
#ggsave(here("figures", "final", "wegu_all.png"), width = 12, height = 8)
...
#ggsave(here("figures", "final", "wegu_cvoi_hCode.png"), width = 12, height = 8)

## look at priority rankings
median_cvoi <- median(wegu_df$mean_cvoi)
median_red <- median(wegu_df$mean_red, na.rm = TRUE)

wegu_rankings <- wegu_df
wegu_rankings$quadrat <- with(wegu_df, ifelse(mean_cvoi >= median_cvoi & mean_red >= median_red, "Highest priority",
...
ggsave(here("figures", "final", "wegu_all_grey.png"), width = 12, height = 8)
